...
//		 destoryDeadPro();
	...
	private void AutoDecreseTime() {
		pcbRunning = manger.getAllRunningProcess();
		// algo = new Algorithm();
		int pid = algo.getOneRunningProcess(pcbRunning);
		if (pid != -1) {
			queue.getPcb(pid).decreaseTime(5000);
			// update table
